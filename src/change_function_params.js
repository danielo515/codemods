...
const inspect = (x) => console.log(util.inspect(x, { depth: 5 }));
module.exports = function (file, api) {
  ...
  return root
    ...
    .forEach((path) => {
      const paramNames = path.node.params.map((x) => j.identifier(x.name));
      ...
      root
        ...
        .forEach(({ node }) => {
          ...
          node.arguments = [
            j.objectExpression(
              args.map((arg, i) => j.objectProperty(paramNames[i], arg))
            ),
          ];
        });
    })
